...
await mkdir('dist/fontawesome/css',{recursive:true});await mkdir('dist/fontawesome/webfonts',{recursive:true});
const fontAssets=[['css/all.min.css','text/css'],['css/solid.css','text/css'],...['fa-solid-900','fa-regular-400','fa-brands-400','fa-v4compatibility'].map(name=>[`webfonts/${name}.woff2`,'font/woff2'])];
for(const [file] of fontAssets)await copyFile(`fontawesome/${file}`,`dist/fontawesome/${file}`);
...
assets.push(...fontAssets.map(([file,mime])=>[`fontawesome/${file}`,`/fontawesome/${file}`,mime]));
