...
setInterval(async()=>{if(!token||polling)return;polling=true;try{const prev=status.settingsRevision;status=await api('status');recordStatus(status);loadLayout();applyWebTheme();$('#connection').textContent=status.connected?'● เชื่อมต่อแล้ว':'● Access Point';if(status.settingsResult&&prev!==status.settingsRevision)notice(status.settingsResult,/failed|Invalid|Cannot|busy/i.test(status.settingsResult));if(page==='dashboard')$$('[data-value]').forEach(el=>el.innerHTML=widgetValue(el.dataset.value));if(page==='wifi')updateWifi();if(page==='files')updateAudioImportStatus();if(page==='ota'){const ota=await api('ota');if($('#otaState')){$('#otaState').textContent=ota.error||({idle:'พร้อมอัปเดต',connecting:'กำลังเชื่อมต่อ',uploading:'กำลังรับไฟล์',downloading:'กำลังดาวน์โหลด',rebooting:'อัปเดตสำเร็จ กำลังเริ่มใหม่',error:'อัปเดตไม่สำเร็จ'}[ota.state]||ota.state);if(ota.total&&!xhr)$('#transferProgress').value=ota.done/ota.total*100;}}}catch(e){$('#connection').textContent='○ ขาดการเชื่อมต่อ';}finally{polling=false;}},2000);
