...
bool isConnectSDcard=true,isPlayingAudio=false;namespace app{struct {bool isRecordingMode=false;} runtime;}
...
struct {int calls=0;void loop(){++calls;}} audio;
'''
...
int main(){
 ...
 now=100;serviceMusic();now=130;serviceMusic();now=135;serviceMusic();assert(audio.calls==3&&getMusicMaxServiceGapMs()==30);musicLastService=0;now=10000;serviceMusic();assert(getMusicMaxServiceGapMs()==30);
...
'''
out=root/'.pio/music-service-test';out.mkdir(exist_ok=True);(out/'main.cpp').write_text(code,encoding='utf8')
