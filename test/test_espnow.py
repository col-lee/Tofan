...
portal=(root/'src/network/WebPortal.cpp').read_text(encoding='utf8')
assert portal.count('AsyncURIMatcher::exact("/api/espnow")') == 2
assert 'AsyncURIMatcher::exact("/api/espnow/send")' in portal
assert 'hasParam("text",true)' in portal and 'queued=espnow::sendText(mac,text)' in portal
assert 'if(!queued){reply(r,409' in portal
slave=(root/'examples/espnow_slave/espnow_slave.ino').read_text(encoding='utf8')
assert 'Serial.write(packet.wire+espnow::Header,length)' in slave
...
int received=0,textReceived=0;std::string receivedBytes,receivedText;
void onPacket(const uint8_t*,const uint8_t*p,size_t n){receivedBytes.assign((const char*)p,n);++received;}
void onText(const uint8_t*,const char*p,size_t n){receivedText.assign(p,n);assert(p[n]=='\0');++textReceived;}
int main(){
 ...
 espnow::begin();assert(!status()["enabled"]);assert(!espnow::submitConfig(""));assert(status()["revision"]==0&&status()["configRevision"]==0&&status()["sendRevision"]==0);assert(status()["configError"]==""&&status()["sendError"]=="");
 String config=R"({"enabled":true,"peers":[{"name":"Lamp","mac":"02:11:22:33:44:55","enabled":true}]})";
 assert(espnow::submitConfig(config));espnow::service();assert(status()["ready"]&&radioPeers.size()==1&&radioPeers[0].channel==0);assert(persisted==config);assert(!status()["peers"][0]["online"]);assert(status()["revision"]==1&&status()["configRevision"]==1&&status()["sendRevision"]==0);
 espnow::mac("02:11:22:33:44:55",m);uint8_t payload[]={0,255,42};assert(espnow::sendPacket(m,payload,3));espnow::service();assert(espnow::decode(lastWire.data(),lastWire.size(),type,seq,size)&&type==espnow::Data&&size==3);assert(status()["revision"]==2&&status()["configRevision"]==1&&status()["sendRevision"]==1);
 txCallback(m,0);espnow::service();assert(status()["peers"][0]["tx"]==1&&!status()["peers"][0]["online"]);
 String hello="Hello slave";assert(espnow::sendText(m,hello));espnow::service();assert(lastWire.size()==hello.size()&&!memcmp(lastWire.data(),hello.data(),hello.size()));
 txCallback(m,0);espnow::service();assert(espnow::sendText(m,"Text from master"));espnow::service();assert(lastWire.size()==16&&!memcmp(lastWire.data(),"Text from master",16));
 txCallback(m,0);espnow::service();String tooLong(201,'x');assert(!espnow::sendText(m,tooLong));assert(!espnow::sendText(m,""));
 espnow::setReceiver(onPacket);espnow::setTextReceiver(onText);uint8_t rawReply[]={'R','e','p','l','y'};rxCallback(m,rawReply,sizeof(rawReply));espnow::service();assert(received==1&&textReceived==1&&receivedBytes=="Reply"&&receivedText=="Reply"&&status()["peers"][0]["payloadHex"]=="5265706C79");
 n=espnow::encode(wire,espnow::Data,8,payload,3);rxCallback(m,wire,n);espnow::service();assert(received==2&&textReceived==2&&receivedBytes==std::string("\0\xff*",3)&&receivedText==std::string("\0\xff*",3)&&status()["peers"][0]["online"]&&status()["peers"][0]["payloadHex"]=="00FF2A");
 uint8_t unknown[6]={2,1,1,1,1,1};rxCallback(unknown,wire,n);wire[2]=9;rxCallback(m,wire,n);espnow::service();assert(received==2);
 failSend=true;assert(espnow::sendText(m,"radio failure"));espnow::service();assert(status()["error"]=="Send failed: mock failure"&&status()["sendError"]=="Send failed: mock failure");failSend=false;
 clockMs=16002;espnow::service();assert(!status()["peers"][0]["online"]);txCallback(m,1);espnow::service();assert(status()["peers"][0]["failed"]==2);
 failSave=true;assert(espnow::submitConfig(R"({"enabled":false,"peers":[]})"));espnow::service();assert(status()["enabled"]&&status()["peers"].size()==1&&status()["configError"]=="Cannot save ESP-NOW settings");failSave=false;
 ...
 std::cout<<"PASS: ESP-NOW text/binary send and receive, protocol bounds, persistent CRUD, malformed frames, radio ACK, timeout and Wi-Fi restart\n";
}
'''
