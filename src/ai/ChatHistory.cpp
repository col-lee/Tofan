...
void ChatHistory::begin(){
    ...
    mutex=xSemaphoreCreateMutex();queue=xQueueCreate(8,sizeof(chat::Entry*));incoming=xQueueCreate(chat::IncomingLimit,sizeof(uint8_t));incomingFree=xQueueCreate(chat::IncomingLimit,sizeof(uint8_t));
    ...
    if(!assembly||!incomingPool||!mutex||!queue||!incoming||!incomingFree||!recent||!index){
        ...
        if(queue){vQueueDelete(queue);queue=nullptr;}if(incoming){vQueueDelete(incoming);incoming=nullptr;}if(incomingFree){vQueueDelete(incomingFree);incomingFree=nullptr;}
        if(mutex){vSemaphoreDelete(mutex);mutex=nullptr;}
        memory::release(assembly);assembly=nullptr;memory::release(incomingPool);incomingPool=nullptr;memory::release(recent);recent=nullptr;memory::release(index);index=nullptr;
        return;
    }
    for(uint8_t slot=0;slot<chat::IncomingLimit;++slot)xQueueSend(incomingFree,&slot,0);
    ...
    if(!buffer||!entry){memory::release(buffer);memory::release(entry);file.close();error="History load allocation failed";damaged=true;return;}
    ...
    memory::release(entry);memory::release(buffer);file.close();
...
void ChatHistory::transcript(bool model,const char* text){
    if(resetPending||!assembly||!recent||!queue||!incoming||!incomingFree||!incomingPool||!text||!text[0])return;
    ...
    if(uxQueueMessagesWaiting(incomingFree)<=chat::FinishReserve){++dropped;return;}
    uint8_t slot=0;if(xQueueReceive(incomingFree,&slot,0)!=pdTRUE){++dropped;return;}
    Incoming& e=incomingPool[slot];e={};e.model=model;
    const size_t length=strlen(text),copied=length>chat::TextLimit?chat::TextLimit:length;
    memcpy(e.text,text,copied);e.text[copied]=0;e.truncated=length>copied;
    ++receivedChunks;receivedBytes.fetch_add(static_cast<uint32_t>(copied));
    if(xQueueSend(incoming,&slot,0)!=pdTRUE){xQueueSend(incomingFree,&slot,0);++dropped;}
}
void ChatHistory::commit(chat::Entry& e,bool partial){
    ...
    if(pending){*pending=e;if(xQueueSend(queue,&pending,0)!=pdTRUE){memory::release(pending);++dropped;}}else ++dropped;
    e={};
}
void ChatHistory::finishMask(uint8_t mask,bool partial){
    if(resetPending||!assembly||!recent||!queue||!incoming||!incomingFree||!incomingPool)return;
    ...
    uint8_t slot=0;if(xQueueReceive(incomingFree,&slot,pdMS_TO_TICKS(40))!=pdTRUE){++dropped;++boundaryDrops;return;}
    Incoming& e=incomingPool[slot];e={};e.finishMask=mask;e.partial=partial;
    if(xQueueSend(incoming,&slot,0)!=pdTRUE){xQueueSend(incomingFree,&slot,0);++dropped;}
}
void ChatHistory::service(){
    if(!assembly||!recent||!queue||!incoming||!incomingFree||!incomingPool||resetPending)return;
    Lock lock(mutex,0);if(!lock.ok)return;
    uint8_t slot=0;for(int i=0;i<chat::IncomingLimit&&xQueueReceive(incoming,&slot,0)==pdTRUE;++i){
        Incoming& e=incomingPool[slot];
        if(e.finishMask){if(e.finishMask&FinishUser)commit(assembly[0],e.partial);if(e.finishMask&FinishModel)commit(assembly[1],e.partial);}
        else {auto& part=assembly[e.model?1:0];chat::append(part,e.text);part.truncated|=e.truncated;assembly[1].model=true;}
        e={};xQueueSend(incomingFree,&slot,0);
    }
    if(millis()-lastAttempt<100)return;lastAttempt=millis();
    ...
    xQueueReceive(queue,&pending,0);memory::release(pending);
}
void ChatHistory::status(JsonDocument& d) const {
    ...
    d["full"]=full;d["damaged"]=damaged;d["truncated"]=truncated;d["dropped"]=dropped.load();d["boundaryDrops"]=boundaryDrops.load();d["error"]=error;
    d["resetting"]=resetPending.load();d["resetRevision"]=resetRevision;d["pending"]=queue?uxQueueMessagesWaiting(queue):0;
    d["receivedChunks"]=receivedChunks.load();d["receivedBytes"]=receivedBytes.load();d["incoming"]=incoming?uxQueueMessagesWaiting(incoming):0;
    d["assemblingUserBytes"]=assembly?assembly[0].length:0;d["assemblingModelBytes"]=assembly?assembly[1].length:0;
...
String ChatHistory::page(unsigned before,bool includeMessages){
    ...
    if(persistent&&end){
        Lock sd(sdSemaphore,50);if(!sd.ok||!isConnectSDcard)d["error"]="SD unavailable or busy";
            ...
            else {auto* buffer=static_cast<char*>(memory::allocate(4097));
                ...
                }memory::release(buffer);}file.close();}}
...
void ChatHistory::reset(){
    if(!resetPending)return;if(!mutex||!assembly||!recent||!queue||!incoming||!incomingFree||!incomingPool){error="History is unavailable";resetPending=false;return;}Lock lock(mutex,100);if(!lock.ok)return;
    ...
    chat::Entry* entry=nullptr;if(queue)while(xQueueReceive(queue,&entry,0)==pdTRUE)memory::release(entry);
    xQueueReset(incoming);xQueueReset(incomingFree);for(uint8_t slot=0;slot<chat::IncomingLimit;++slot){incomingPool[slot]={};xQueueSend(incomingFree,&slot,0);}
    receivedChunks=0;receivedBytes=0;boundaryDrops=0;
