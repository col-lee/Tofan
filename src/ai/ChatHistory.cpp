...
void ChatHistory::begin()
{
    ...
    if (!assembly || !mutex || !queue || !incoming || !recent || !index)
    {
...
{
    if (resetPending || !assembly || !recent || !queue || !incoming || !text || !text[0])
        return;
    ++receivedChunks;
    // Never wait for the SD/history mutex on the WebSocket/audio worker.
    auto *e = static_cast<Incoming *>(memory::zeroAllocate(1, sizeof(Incoming)));
    if (!e)
    {
        ++dropped;
        return;
    ...
    if (xQueueSend(incoming, &e, 0) != pdTRUE)
    {
        free(e);
        ++dropped;
    }
...
{
    if (resetPending || !assembly || !recent || !queue || !incoming)
        return;
    auto *e = static_cast<Incoming *>(memory::zeroAllocate(1, sizeof(Incoming)));
    if (!e)
    {
...
{
    if (!assembly || !recent || !queue || !incoming || resetPending)
        return;
    Lock lock(mutex, 0);
    if (!lock.ok)
        return;
    Incoming *e = nullptr;
    for (int i = 0; i < 16 && xQueueReceive(incoming, &e, 0) == pdTRUE; ++i)
    {
        if (e->finish)
        {
...
String ChatHistory::page(unsigned before, bool includeMessages)
{
    ...
    if (persistent && end)
    {
        ...
        else
        {
            ...
            {
                auto *buffer = static_cast<char *>(heap_caps_malloc(4097, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
                if (!buffer)
...
void ChatHistory::reset()
{
    if (!resetPending)
        return;
    if (!mutex || !assembly || !recent || !queue)
    {
    ...
    if (queue)
        while (xQueueReceive(queue, &entry, 0) == pdTRUE)
            free(entry);
    Incoming *message = nullptr;
    if (incoming)
        while (xQueueReceive(incoming, &message, 0) == pdTRUE)
            free(message);
    receivedChunks = 0;
    assembly[0] = {};
    assembly[1] = {};
