...
#include <atomic>
namespace chat {
constexpr size_t TextLimit=1536,RecentLimit=8,ArchiveLimit=1024*1024,RecordLimit=1000,ContextLimit=8192,IncomingLimit=24,FinishReserve=2;
...
class ChatHistory {
    enum : uint8_t { FinishUser=1,FinishModel=2,FinishBoth=3 };
    struct Incoming {uint8_t finishMask=0;bool partial=false,model=false,truncated=false;char text[chat::TextLimit+1]{};};
    struct Index {uint32_t offset,length;};
    SemaphoreHandle_t mutex=nullptr;QueueHandle_t queue=nullptr;
    QueueHandle_t incoming=nullptr,incomingFree=nullptr;
    Incoming* incomingPool=nullptr;
    ...
    String error;
    std::atomic<unsigned> dropped{0},boundaryDrops{0};std::atomic<bool> resetPending{false};
    std::atomic<unsigned> receivedChunks{0};std::atomic<uint32_t> receivedBytes{0};
    void remember(const chat::Entry& entry);
    void commit(chat::Entry& entry,bool partial);
    void status(JsonDocument& d) const;
    void finishMask(uint8_t mask,bool partial);
public:
    void begin();
    void transcript(bool model,const char* text);
    void finish(bool partial=false){finishMask(FinishBoth,partial);}
    void finishUser(bool partial=false){finishMask(FinishUser,partial);}
    void finishModel(bool partial=false){finishMask(FinishModel,partial);}
    void service();
