...
namespace chat
{
    ...
    struct Entry
    {
        uint32_t id = 0, time = 0, uptime = 0;
        bool model = false, partial = false, truncated = false;
        uint16_t length = 0;
    ...
    inline void append(Entry &entry, const char *text)
    {
        ...
        for (size_t i = 0; text[i];)
        {
            ...
            for (size_t j = 1; j < n; ++j)
                if (!text[i + j] || (static_cast<uint8_t>(text[i + j]) & 0xc0) != 0x80)
                {
                    complete = false;
                    break;
                }
            ...
            if (entry.length + n > TextLimit)
            {
                entry.truncated = true;
                break;
            }
            for (size_t j = 0; j < n; ++j)
                entry.text[entry.length++] = text[i++];
...
class ChatHistory
{
    struct Incoming
    {
        bool finish = false, partial = false, model = false, truncated = false;
        char text[chat::TextLimit + 1]{};
    };
    struct Index
    {
    ...
    chat::Entry *recent = nullptr;
    Index *index = nullptr;
    chat::Entry *assembly = nullptr;
    unsigned recentCount = 0, recentStart = 0, count = 0;
    uint32_t bytes = 0, nextId = 1, lastAttempt = 0, resetRevision = 0;
    bool persistent = false, damaged = false, full = false, truncated = false;
    String error;
    std::atomic<unsigned> dropped{0};
    std::atomic<bool> resetPending{false};
    std::atomic<unsigned> receivedChunks{0};
