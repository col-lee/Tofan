...
bool textQueued = espnow::sendText(slave, "Hello slave");
...
`sendPacket()` accepts binary payloads of 0–200 bytes and adds the ToFan protocol header. `sendRaw()` and `sendText()` send exactly 1–200 payload bytes without that header, so an ordinary ESP-NOW receiver gets the original UTF-8 text. `true` means queued, not delivered; inspect TX/RX/failure counters. The peer must already be enabled and registered. Wi-Fi callbacks only copy packets to a bounded queue; payload queues and peer storage prefer PSRAM. No additional task stack is allocated. ESP-NOW is stopped before the network worker changes Wi-Fi mode, then peers are registered again.

Authenticated routes are `GET /api/espnow`, `POST /api/espnow` (form field `values`, JSON containing boolean `enabled` and `peers` array), and `POST /api/espnow/send`. The send route accepts `mac` with either UTF-8 `text` or hexadecimal `hex`; the legacy `hex` request remains supported. Mutation routes require the existing portal client header and reject firmware-update overlap. The GET route uses `Cache-Control: no-store`. Settings/send requests return 202 only when work was queued. Poll `configRevision` and read `configError` for a settings result, or poll `sendRevision` and read `sendError` for a send result. Aggregate `revision` and `error` remain available for diagnostics and compatibility.

All three ESP-NOW routes use exact URI matching. This is required because the ESPAsyncWebServer compatibility matcher treats `/api/espnow` as matching child paths such as `/api/espnow/send`; without an exact matcher, a send request can be misrouted to the configuration handler with an empty `values` field. Empty configuration bodies are rejected before entering the manager queue.
...
Open `examples/espnow_slave/espnow_slave.ino` with its adjacent `EspNowProtocol.hpp` using ESP32 Arduino 2.x (ESP-IDF 4.4). Set `masterMac` from the signed-in ToFan page and `channel` to the displayed channel. Flash the slave and add the slave STA MAC printed by its Serial monitor to ToFan. The example answers heartbeat, prints Data as both hexadecimal and UTF-8 text, and echoes the payload. Replace the echo handler with your sensor/actuator logic. Arduino 3.x has different ESP-NOW callback signatures and requires adapting the example.
